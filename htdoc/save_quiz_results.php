...
error_log("DEBUG: Raw POST data received: " . json_encode($data)); // NEW DEBUG: Log raw data
...
$quiz_duration_seconds = $data['quiz_duration_seconds'] ?? null; // Lấy từ 'quiz_duration_seconds'
...
try {
    ...
    if ($admin_id !== null) {
        ...
        // Tạo thông báo chi tiết dưới dạng JSON
        $notification_details = [
            'student_name' => $student_name,
            'correct_answers' => $correct_answers,
            'wrong_answers' => $wrong_answers_count,
            'total_questions' => $total_questions_quizzed,
            'quiz_duration_seconds' => $quiz_duration_seconds, // Thêm thời gian giới hạn
            'wrong_words_details' => [], // MỚI: Danh sách các từ sai chi tiết
        ];

        // Thêm chi tiết từng từ sai vào mảng wrong_words_details
        foreach ($wrong_words_data as $word) {
            $notification_details['wrong_words_details'][] = [
                'english_word' => $word['english_word'] ?? 'N/A', // Sửa thành snake_case
                'correct_meaning' => $word['correct_meaning'] ?? 'N/A', // Sửa thành snake_case
                'student_answer' => $word['student_answer'] ?? 'N/A', // Sửa thành snake_case
            ];
        }

        $notification_message_json = json_encode($notification_details);
        error_log("DEBUG: Notification JSON: " . $notification_message_json); // DEBUGGING
        ...
        if ($stmtNotify === false) {
            ...
            $stmtNotify->bind_param("iss", $admin_id, $notification_message_json, $notification_type);
